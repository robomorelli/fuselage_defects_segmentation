...
def main():
    """
    Main function to launch hyperparameter optimization sweep.
    """
    ...
    parser.add_argument('--sweep_cfg', type=str, default='screening', help='Sweep config file (without .yaml)')
    ...
    parser.add_argument('--sweep_id', type=str, default=None, help='Existing sweep ID')
    ...
    # Update sweep config with runtime parameters
    sweep_config['parameters']['n_gpus'] = {'value': args.ngpus}
    sweep_config['parameters']['ncpus'] = {'value': args.ncpus}

    # If architecture is specified and not in sweep config, add it
