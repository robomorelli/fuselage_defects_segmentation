    ...
    if args.n_splits == 1 and args.test_from_txt_file is not None:
        ...
        if args.fix_val_ratio:
            ...
            if args.test_from_txt_file is not None:
                ...
                raise NotImplementedError('empty test set')
            ...
            all_index = np.arange(len(data))
            remaining_index = [i for i in all_index if i not in test_index]
            remaining_data = [data[i] for i in remaining_index]

            #compute n_splits to get the right number of train and val images
            length_val = len(remaining_data)*args.val_ratio
            n_splits_total = int(len(remaining_data)//length_val)

            kf = KFold(n_splits=n_splits_total, shuffle=True, random_state=seed)
            for i, (train_index, val_index) in enumerate(kf.split(remaining_data)):
                if i >= args.n_splits:
                    break
                # Initialize KFold splitter
                train_dict = {}
